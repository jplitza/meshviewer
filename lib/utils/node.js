define(['snabbdom', 'helper', 'moment'], function (V, helper, moment) {
  ...
  self.showFirmware = function showFirmware(d) {
    return [
      ...
      self.firmwareVersionStatus(d)
    ].filter(function (n) {
      return n !== null;
    }).join(' / ') || undefined;
  };

  // Displays if the firmware Version matches the available one
  self.firmwareVersionStatus = function firmwareVersionStatus(d) {
    if (!(config.firmware || {}).enabled) {
      return null;
    }
    let ret = _.t('firmware.unknown');
    const node_firmware = helper.dictGet(d, ['firmware', 'release']);
    if(firmware) {
      if( firmware["up-to-date"] ) {
        if(firmware["up-to-date"].includes(node_firmware)) {
          ret = _.t('firmware.up-to-date');
        }
        if(firmware["outdated"].includes(node_firmware)) {
          ret = _.t('firmware.outdated');
        }
      }
    }
